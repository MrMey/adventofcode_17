# -*- coding: utf-8 -*-
"""
Created on Fri Dec  8 16:22:22 2017

@author: Mr_Mey
"""

string = """pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
...
# problem 2
def test_anagram(w1,w2):
	if len(w1) != len(w2):
		return False
	for letter in w1:
		if len(w2) == 0:
			return False
		if w2.count(letter)>0:
			ind = w2.index(letter)
			w2 = w2[0:ind] + w2[ind+1:]
	if len(w2) == 0:
		return True
	return False


def test_passphrase2(string):
    cnt = 0
    for test in string:
        res = True
        test = test.split(' ')
        for i in range(len(test)):
            for j in range(i+1,len(test)):
                if test_anagram(test[i],test[j]):
                    res = False
        if res:
            cnt += 1
    return cnt

print(test_passphrase2(string))